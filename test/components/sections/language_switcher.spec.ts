...
test("LanguageSwitcher component", async () => {
    ...
    expect(wrapper.find(".lang").findComponent(Icon).element.innerHTML).toEqual(fa_icon);
    expect(wrapper.find(".lang").findAllComponents(Icon)[1].props("size")).toEqual(14);
    ...
    expect(wrapper.findAllComponents(Icon)[0].element.innerHTML).toEqual(en_icon);
    expect(language.current).toEqual("en");
    
    await wrapper.findAll(".items button")[1].trigger("click");
    expect(wrapper.findAllComponents(Icon)[0].element.innerHTML).toEqual(fa_icon);
